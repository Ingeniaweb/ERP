...
else
{
    ...
    if ($action == 'create' && $usercancreate)
    {
        ...
        print '<form id="form_producto" action="'.$_SERVER["PHP_SELF"].'" method="POST">';
        ...
        // Label
        print '<tr><td class="fieldrequired">'.$langs->trans("Nombre").'</td><td colspan="3"><input name="label" class="minwidth300 maxwidth400onsmartphone" maxlength="255" value="'.dol_escape_htmltag(GETPOST('label')).'"></td></tr>';
...
*/
 ...
 // Stock min level
    ...
    /*    print '<table class="border" width="100%">';
        ...
        print '<input type="button" class="button" value="' . $langs->trans("Create") . '" onClick="guardar();">';

        print ' &nbsp; &nbsp; ';
        print '<input type="button" class="button" value="' . $langs->trans("Cancel") . '" onClick="cerrar_div();">';
        print '</div>';
    ...
    else if ($object->id > 0)
    {
        ...
        if ($action == 'edit' && $usercancreate)
        {
            ...
            print '<form id="form_producto" action="'.$_SERVER['PHP_SELF'].'?id='.$object->id.'" method="POST">'."\n";
            print '<input type="hidden" name="token" value="'.$_SESSION['newtoken'].'">';
        ...
        else
        {
            ...
            if ($showbarcode)
            {
                ...
                if ($action == 'editbarcode')
                {
                    ...
                    print '<form method="post" id="form_producto" action="'.$_SERVER["PHP_SELF"].'?id='.$object->id.'">';
                    print '<input type="hidden" name="token" value="'.$_SESSION['newtoken'].'">';
    ...
    if (empty($reshook))
    {
        if ($usercancreate)
        {
           /* if (! isset($object->no_button_edit) || $object->no_button_edit <> 1) print '<div class="inline-block divButAction"><a class="butAction" href="'.$_SERVER["PHP_SELF"].'?action=edit&amp;id='.$object->id.'">'.$langs->trans("Modify").'</a></div>';
            ...
            $ver="'si'";
            print '<div class="inline-block divButAction"><a class="butAction" onClick="volver('.$object->id.','.$ver.');">'.$langs->trans("Añadir").'</a></div>';
            
            print '<br>';
            print ' <script>
                        window.parent.document.getElementById("nuevo_producto").dataset.id="'.$object->id.'";
                    </script>';
...
if (! empty($conf->global->PRODUCT_ADD_FORM_ADD_TO) && $object->id && ($action == '' || $action == 'view') && $object->status)
{
    ...
    if (!empty($html))
    {
        print '<form id="form_producto" method="POST" action="'.$_SERVER["PHP_SELF"].'?id='.$object->id.'">';
        print '<input type="hidden" name="token" value="'.$_SESSION['newtoken'].'">';
 ...
 */
    ...
    print '<script>
        ...
        $(".login_block").hide();
        function cerrar_div(){
            window.parent.document.getElementById("nuevo_producto").style.display="none";
        }
        function guardar(){
        ...
        function volver(id,elegir){
            var ele=elegir||"si";
            var lugar=window.parent.location.href;
            var tipo="venta";
            ...
            setTimeout(function(){
                $.ajax({
                    ...
                    type: "post",
                    url:   "../dev_op_product.php", 
                    success:  function (respuesta) { 
                            window.parent.document.getElementById("idprod").innerHTML=respuesta;
                            //alert(respuesta);
                    }
                 });

                 location.href="../product/frame_card_nv.php?leftmenu=product&action=create&type=0";
                cerrar_div();
            },2000);
        }
    </script>';
