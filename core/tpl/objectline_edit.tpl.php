...
<tr <?php echo $bc[$var]; ?>>
	...
	{
	?>
		<td align="center"><span>REFERENCIA</span><br /><input id="fourn_ref" name="fourn_ref" class="flat" value="<?php echo $line->ref_fourn; ?>" size="12"></td>
	<?php
	}

	$coldisplay++;
	if ($this->situation_counter == 1 || !$this->situation_cycle_ref) {
		print '<td align="center"><span>IVA</span><br />' . $form->load_tva('tva_tx', $line->tva_tx.($line->vat_src_code?(' ('.$line->vat_src_code.')'):''), $seller, $buyer, 0, $line->info_bits, $line->product_type, false, 1) . '</td>';
	} else {
		print '<td align="center"><span>IVA</span><br /><input size="1" type="text" class="flat right" name="tva_tx" value="' . price($line->tva_tx) . '" readonly />%</td>';
	}

	$coldisplay++;
	print '<td align="center"><span>PRECIO</span><br /><input type="text" class="flat right" size="5" id="price_ht" name="price_ht" value="' . (isset($line->pu_ht)?price($line->pu_ht,0,'',0):price($line->subprice,0,'',0)) . '"';
	...
	<?php if (($line->info_bits & 2) != 2) {
		...
		print '<span>CANTIDAD</span><br /><input size="3" type="text" class="flat right" name="qty" id="qty" value="' . $line->qty . '"';
		if ($this->situation_counter > 1) print ' readonly';
		print '>';
	} else { ?>
		&nbsp;
	...
	<?php if (($line->info_bits & 2) != 2) {
		print '<span>DESCUENTO</span><br /><input size="1" type="text" class="flat right" name="remise_percent" id="remise_percent" value="' . $line->remise_percent . '"';
		if ($this->situation_counter > 1) print ' readonly';
		print '>%';
	} else { ?>
		&nbsp;
