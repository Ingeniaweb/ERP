...
class Consumption extends CommonObject
{
	...
	function correct_stock($productid, $user, $id_entrepot, $nbpiece, $movement, $label='', $price=0, $inventorycode='', $origin_element='', $origin_id=null)
	{
		if ($id_entrepot)
		{
			$this->db->begin();
			...
			$result=$movementstock->_create($user,$productid,$id_entrepot,$op[$movement],$movement,$price,$label,$inventorycode);
	...
	function showformwrite($user,$consotype,$entity,$formproduct,$html,$conf)
	{
		...
		if($right)
		{
				...
				// Warehouse
				print '<tr>';
				print '<td width="10%" class="fieldrequired">'.$langs->trans("Warehouse").'</td>';
				print '<td width="20%">';
				print $formproduct->selectWarehouses(($_GET["dwid"]?$_GET["dwid"]:GETPOST('id_entrepot')),'id_entrepot','',1);
				print '</td>';
				print '<td width="10%" class="fieldrequired">'.$langs->trans("Product").'</td>';
				print '<td width="20%">';
	...
	function showformview($user,$consotype,$entity,$formproduct,$html,$conf)
	{
		...
		if ($resql)
		{
			...
			print '<form method="POST" action="'.$_SERVER["PHP_SELF"].'?id='.$id.'&type=projet">';
